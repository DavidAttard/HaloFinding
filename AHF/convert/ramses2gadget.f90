...
program ramses2gadget  
  ...
  real(kind=8), parameter :: kpc_in_cm = 3.0856776d+21 ! amr/constants.f90
  ...
  integer(1), dimension(:), allocatable :: ramsespart_fam  ! LC
  integer(1), parameter :: FAM_STAR = 2, FAM_DM = 1, FAM_DEBRIS = 4
...
#ifdef RAMSES_CRAL
  read(info_file,*)              ! LC -- for ramses_cral skip U_precision
#endif
...
!!! READING DM + STAR PARTICLES !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 ! [ RF ] note: in this branch neither ndmpart nor nstarpart are yet set correctly
     ...
     if (stars) allocate(ramsespart_fam(1:nramsespart)) ! ( ramsespart_age(1:nramsespart) )
     ...
     if (stars) then
        ...
        read (part_file) ramsespart_fam ! skip family
     end if
     !    read (part_file) ! skip tag
...
!!! WRITE DATA INTO OUTPUT FILE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     if (gas) then
        if (stars) then
           ...
           do i=1,nramsespart
              if ( ramsespart_fam(i) == FAM_DM ) then
              ...
              ! LC counting debris as stars, since not sure where else
              ! they should go. Perhaps gas, but they're pressureless.
              else if ((ramsespart_fam(i) == FAM_STAR) .or. (ramsespart_fam(i) == FAM_DEBRIS)) then
                 ! particle is a star
                 istar = istar + 1
	   ...
	   ndmpart=idm;     ! [ RF ]
         ...
         ! check if particle count is ok
           if ( idm /= ndmpart .or. istar /= nstarpart .or. idm + istar /= nramsespart) then
              if (icpu==0) then
                 write(*,*) 'Error in input files: counted wrong amount of dm and star particles'
                 write(*,*) 'idm, ndmpart', idm, ndmpart
                 write(*,*) 'istar, nstarpart', istar, nstarpart
                 write(*,*) 'istar+idm, nramsespart', istar+idm, nramsespart
                 end if
                 call terminate(4)            
           end if
    ...
    ! deallocate ramses data arrays
           deallocate (ramsespart_pos,ramsespart_vel,ramsespart_m,ramsespart_id,ramsespart_fam)
...
!!! UPDATE GADGET HEADER !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if (icpu == 0) then
     ...
     write(*,*) 'ndmpart_total=',ndmpart_total
     write(*,*) 'ngaspart_total=',ngaspart_total
     write(*,*) 'nstarpart_total=',nstarpart_total
